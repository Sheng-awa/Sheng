...
- **对弈**：首页「小角落」的对弈卡进 `games.html`——**左侧导航单页**（和小窝同款侧边栏，点象棋/五子棋/围棋就地切换不跳转，移动端变顶栏）。**中国象棋**：完整规则引擎（蹩马腿/塞象眼/炮架/白脸将/将军判定，perft 深度 1~3 = 44/1920/79666 全对，引擎在 `/*ENGINE-START*/` 块内可直接提取到 node 验证）、AI「小圣」= alpha-beta 剪枝 + 子力/位置评估（固定困难 d4）、可选执子红先黑后（执黑棋盘翻转）、悔棋（连撤两步带回滑动画，被吃的子从托盘飞回）/认输/**求和**（小圣评估自己不占优就接受，占优拒绝）/**💡小灯泡**（一局一次，给出最佳着法并在棋盘上画金色箭头）。**右下角战利品托盘**（canvas 加高一条）：被吃的棋子飞进托盘按棋子样子停放，**重开 = 全场棋子各自飞回初始位**（首次加载也是全体从托盘飞出各就各位）。全部动画 rAF 驱动 + setTimeout 兜底，减动效直跳；120 回合无吃子判和。五子棋/围棋是在建占位，加新棋 = 导航加项 + stage 加段
