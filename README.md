...
- **音乐**：`music.html` 是双音源播放器——内置本地曲目 + 网易云歌单（页面里粘贴歌单/歌曲链接或 ID 即可，封面歌词自动来，经 [Meting 公共 API](https://github.com/injahow/meting-api) 解析，版权歌可能只有试听）。本地曲目分两层：`assets/music/local/` 是小圣自己的歌（网易云 ncm 经 [ncmdump](https://github.com/taurusxin/ncmdump) 转 mp3，配网易云 JSON 歌词），`assets/music/` 根下是三首 Kevin MacLeod（[incompetech.com](https://incompetech.com)）CC BY 4.0 BGM。加歌：文件丢进对应目录，在 `assets/js/music.js` 顶部 `LOCAL_TRACKS` 里加一行
