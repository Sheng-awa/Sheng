...
- **对弈**：首页「小角落」的对弈卡进 `games.html`——**左侧导航单页**（和小窝同款侧边栏，点象棋/五子棋/围棋就地切换不跳转，移动端变顶栏；侧栏「中国象棋」logo 是红色圆底白字「帅」徽标）。**中国象棋**：完整规则引擎（蹩马腿/塞象眼/炮架/白脸将/将军判定，perft 深度 1~3 = 44/1920/79666 全对，引擎在 `/*ENGINE-START*/` 块内可直接提取到 node 验证）、AI「小圣」= alpha-beta 剪枝 + 子力/位置评估（固定困难 d4）、可选执子红先黑后（执黑棋盘翻转）、悔棋（连撤两步带回滑动画，被吃的子从托盘飞回）/认输/**求和**（小圣评估自己不占优就接受，占优拒绝）/**💡小灯泡**（一局一次，给出最佳着法并在棋盘上画金色箭头）。**右下角战利品托盘**（canvas 加高一条）：被吃的棋子飞进托盘按棋子样子停放，**重开 = 全场棋子各自飞回初始位**（首次加载也是全体从托盘飞出各就各位）。**五子棋**：15×15 棋盘 + 左右两只木碗棋盒（黑左白右）、落子从棋盒飞入、悔棋/重开时棋子飞回棋盒（渐隐落碗）、五连高亮圈、执子选择，AI「小圣」= 单步启发式（攻/防模式分：活四 1e6/冲四 1e5/活三 5e4…候选点取近 2 格，攻×1.05+防）。**围棋**（2026-08-11 新增，国际象棋已删）：**13×13** Canvas 棋盘 + 完整基础规则（气/提子/禁着点自杀禁/打劫 = 全局同型禁），引擎在 `/*GO-START*/` 块内可直接提取到 node 验证（node 单测 8 项全过：提 1/提 2/角上提 3/自杀禁/连子合法/劫扑/领地混合界 dame）；AI「小圣」= 启发式（吃子 900/子 > 叫吃 320 > 近处扩展 8 + 开局星位天元偏好 40 + 随机抖动 60）；**过一手（Pass）按钮**：双方连续 Pass → 终局**自动粗算**（空白领地 flood fill 归属 + 提子数，结果「黑 N 目 · 白 M 目 · 胜者 ✿」，仅供娱乐）；执黑先/执白后可选、悔棋（连撤玩家 + AI 两步含提子恢复）/重开/认输；落子 pop 动画 + 提子淡出 + 最后一步红色标记，减动效直跳；深浅主题两套木色（白天暖柚木 / 夜晚深棕亮线）。全部动画 rAF 驱动 + setTimeout 兜底，减动效直跳；象棋 120 回合无吃子判和
