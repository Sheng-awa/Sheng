...
- **星空模式**：主题按钮旁多一颗 **✦** 星星按钮（导航右下角，收起态两钮竖排）——点击把全站切成深蓝紫星空：星云渐变背景淡入 + Canvas 星星（白/淡粉/淡紫/淡金混色，随视口自适应密度，每颗独立 sin 相位闪烁 + 极慢漂移，`prefers-reduced-motion` 时静止不闪）。**开星空自动强制深色主题**（星星要衬深色底），此时点 ☾/☀ = 退出星空 + 偏好主题翻转一次；关闭星空恢复原主题。选择记 localStorage `sheng-star`。目前只首页生效（按钮只在这），以后可扩展全站（子页面需各自读 `sheng-star` + 叠背景层）
