...
- **对弈**：首页「小角落」的对弈卡进 `games.html`——**左侧导航单页**（和小窝同款侧边栏，点象棋/五子棋/国际象棋/围棋就地切换不跳转，移动端变顶栏；侧栏「中国象棋」logo 是红色圆底白字「帅」徽标，不是国际象棋 emoji）。**中国象棋**：完整规则引擎（蹩马腿/塞象眼/炮架/白脸将/将军判定，perft 深度 1~3 = 44/1920/79666 全对，引擎在 `/*ENGINE-START*/` 块内可直接提取到 node 验证）、AI「小圣」= alpha-beta 剪枝 + 子力/位置评估（固定困难 d4）、可选执子红先黑后（执黑棋盘翻转）、悔棋（连撤两步带回滑动画，被吃的子从托盘飞回）/认输/**求和**（小圣评估自己不占优就接受，占优拒绝）/**💡小灯泡**（一局一次，给出最佳着法并在棋盘上画金色箭头）。**右下角战利品托盘**（canvas 加高一条）：被吃的棋子飞进托盘按棋子样子停放，**重开 = 全场棋子各自飞回初始位**（首次加载也是全体从托盘飞出各就各位）。**五子棋**：15×15 棋盘 + 左右两只木碗棋盒（黑左白右）、落子从棋盒飞入、悔棋/重开时棋子飞回棋盒（渐隐落碗）、五连高亮圈、执子选择，AI「小圣」= 单步启发式（攻/防模式分：活四 1e6/冲四 1e5/活三 5e4…候选点取近 2 格，攻×1.05+防）。**国际象棋**（2026-08-10 新增）：8×8 棋盘 + 完整规则（王车易位/吃过路兵/升变自动变后/将死/逼和，perft 深度 1~3 = 20/400/8902 全对，引擎在 `/*CHESS-START*/` 块内可直接提取到 node 验证）、AI「小圣」= alpha-beta 剪枝（深度 3）+ 子力/位置评估（吃子优先排序、开局轻微随机）、执白/执黑可选（执黑小圣先手）、悔棋（连撤玩家 + AI 两步，**将死后也能悔棋**）/重开/认输；落子有选中、可走提示、最后一步高亮动画，**重开/开局有棋子从右下角错峰飞回原位的摆棋动画**（参考中国象棋）；棋子用 Unicode 实心符号（黑白靠颜色区分）。全部动画 rAF 驱动 + setTimeout 兜底，减动效直跳；象棋 120 回合无吃子判和。围棋是在建占位，加新棋 = 导航加项 + stage 加段
