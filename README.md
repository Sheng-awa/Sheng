...
- **音乐**：`music.html` 是双音源播放器——本地歌单（小圣自己下载的歌，在 `assets/music/local/`，ncm 可用 [ncmdump](https://github.com/taurusxin/ncmdump) 转 mp3）+ 网易云歌单（页面里粘贴歌单/歌曲链接或 ID 即可，封面歌词自动来，经 [Meting 公共 API](https://github.com/injahow/meting-api) 解析，版权歌可能只有试听）。页面背景 `assets/img/music-bg.jpg` 取自 Wallpaper Engine 场景壁纸（workshop 3448877775，RePKG 解包 scene.pkg 后按图层坐标手工拼合的静帧）。加歌：文件丢进 `assets/music/local/`，在 `assets/js/music.js` 顶部 `LOCAL_TRACKS` 里加一行
