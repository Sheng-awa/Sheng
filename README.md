...
- **对弈**：首页「小角落」的对弈卡进 `games.html`——**左侧导航单页**（和小窝同款侧边栏，点象棋/五子棋/围棋就地切换不跳转，移动端变顶栏；侧栏「中国象棋」logo 是红色圆底白字「帅」徽标）。**中国象棋**：完整规则引擎（蹩马腿/塞象眼/炮架/白脸将/将军判定，perft 深度 1~3 = 44/1920/79666 全对，引擎在 `/*ENGINE-START*/` 块内可直接提取到 node 验证）、AI「小圣」= alpha-beta 剪枝 + 子力/位置评估（固定困难 d4）、可选执子红先黑后（执黑棋盘翻转）、悔棋（连撤两步带回滑动画，被吃的子从托盘飞回）/认输/**求和**（小圣评估自己不占优就接受，占优拒绝）/**💡小灯泡**（一局一次，给出最佳着法并在棋盘上画金色箭头）。**右下角战利品托盘**（canvas 加高一条）：被吃的棋子飞进托盘按棋子样子停放，**重开 = 全场棋子各自飞回初始位**（首次加载也是全体从托盘飞出各就各位）。**五子棋**：15×15 棋盘 + 左右两只木碗棋盒（黑左白右）、落子从棋盒飞入、悔棋/重开时棋子飞回棋盒（渐隐落碗）、五连高亮圈、执子选择，AI「小圣」= 单步启发式（攻/防模式分：活四 1e6/冲四 1e5/活三 5e4…候选点取近 2 格，攻×1.05+防）。**围棋**（2026-08-11 新增，国际象棋已删）：**19×19** Canvas 棋盘（872×660，两侧 106px 留白放木碗棋盒——黑盒在左、白盒在右，和五子棋同款样式）+ 完整基础规则（气/提子/禁着点自杀禁/打劫 = 全局同型禁），引擎在 `/*GO-START*/` 块内可直接提取到 node 验证（node 单测 8 项全过：提 1/提 2/角上提 3/自杀禁/连子合法/劫扑/领地混合界 dame）；AI「小圣」= 启发式（吃子 900/子 > **逃子救被叫吃组 420+组大小×26** > 叫吃 340 > **落子后己组气数 ×14** > 边角 26/14 > 近处扩展 10 + 开局星位 40 + 随机 26，**≥12 手后候选剪枝到棋子 2 格内 + 星位**——集中作战才能围地吃子），**AI 明显落后会认输**（≥36 手且目差 ≥30 →「小圣认输了 ✿ 你赢啦！」）；**右侧对局信息面板**：两行对称排版——「提子 黑 0 : 0 白」+「实时胜率 黑 50% : 50% 白」，底部实时胜率条（围空粗算 + 提子 + 盘面子比例，仅供娱乐）；**过一手（Pass）按钮**：双方连续 Pass → 终局**自动粗算**（空白领地 flood fill 归属 + 提子数，结果「黑 N 目 · 白 M 目 · 胜者 ✿」）；执黑先/执白后可选、悔棋（连撤玩家 + AI 两步含提子恢复）/认输；**动画**：落子从对应棋盒飞入落点（0.26s ease）、提子淡出、**重开 = 全场棋子错峰飞回各自棋盒再开局**（黑回左白回右，渐隐落碗），减动效直跳；深浅主题两套木色（白天暖柚木 #e6bd7a / 夜晚亮木 #6b4e30 + 亮线），**暗色/星空下棋盘加亮木描边保持不透明**。全部动画 rAF 驱动 + setTimeout 兜底，减动效直跳；象棋 120 回合无吃子判和
