...
├── photo/              # 📷 照片源文件夹（备用图床，现在相册页还没引用它）
│   ├── web/            #   （脚本生成）压缩后的网页版，别手改
│   └── photos.json     #   （脚本生成）照片清单，可手改 caption
├── tools/update-photos.py  # 照片压缩管线（备用）：扫描 photo/ → 压缩重命名 → 生成清单
├── 更新相册.bat        # 双击 = 跑上面那个脚本
├── assets/
