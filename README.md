...
├── photos.html         # 相册页（日系手帐风：拍立得瀑布流 + lightbox）
├── photo/              # 📷 照片源文件夹——直接往里塞照片就行
│   ├── web/            #   （脚本生成）压缩后的网页版，别手改
│   └── photos.json     #   （脚本生成）照片清单，可手改 caption
├── tools/update-photos.py  # 相册管线：扫描 photo/ → 压缩重命名 → 生成清单
├── 更新相册.bat        # 双击 = 跑上面那个脚本
├── assets/
...
│   ├── js/album.js     # 相册页：读 photos.json 渲染瀑布流、随机倾斜、lightbox
...
- **相册**：首页「小角落」的相册卡进 `photos.html`——日系手帐风：米白方格纸背景 + 手绘贴纸，拍立得卡片（白相纸边、底部手写体 caption/日期用"志莽行"字体、随机倾斜 -3°~3°、顶部和纸胶带三色轮换），悬停回正放大，点击开 lightbox（‹ › 切换、Esc/点背景关闭、方向键可用）。瀑布流是 `album.js` 按"最矮列优先"JS 分列（不用 CSS columns——它会让卡片内绝对定位的胶带错位）。**加照片：图片丢进 `photo/` 文件夹，双击 `更新相册.bat`（或 `python tools/update-photos.py`），再 push 就行**——脚本自动按时间排序、重命名 p00X、压缩到长边 1600（gif 原样保留）、重建 `photo/photos.json`；删掉的照片会自动从清单移除，不会出现 404。照片底下那句话：改 `photo/photos.json` 里的 `caption`（空的话显示 ✿）
